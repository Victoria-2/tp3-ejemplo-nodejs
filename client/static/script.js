  ...
  try {
    ...
    if (!response.ok) {
      ...
      tablaAlumnos.innerHTML =
        '<tr><td colspan="5" style="color: red; text-align: center;">❌ Error al traer los datos</td></tr>'
    }
...
async function getAlumnoById () {
  try {
    ...
    if (!response.ok) {
      ...
      tablaAlumnos.innerHTML =
        '<tr><td colspan="6" style="text-align: center; color: red;">Alumno inexistente</td></tr>'
      return
    }
...
function renderizarTabla (alumnos) {
  if (alumnos.length === 0) {
    tablaAlumnos.innerHTML =
      '<tr><td colspan="5" style="text-align: center;">No hay alumnos registrados en el sistema.</td></tr>'
    return
  }
